...
* keeps track of submitter in each slot -> When a transaction is submitted, the contract runs the deterministic leader election algorithm (e.g., the XOR check) on-chain. It verifies that the transaction sender (msg.sender) is the one and only legitimate leader for the current slot.
